package com.uclab.ER.audio.classification;
...
public class Classification{
	...
	public Classification(String filename, int FEATURES_NUM, String[] label,
	...
	private Instances createLearningDataSet(FastVector allAttributes,
		...
		try {
			...
			while ((record = in.readLine()) != null) {
				...
				for (int i = 0; i < FEATURES_NUM; i++) {
					features[i] = Double.valueOf(dataset[i]);
				}
				String[] predictEmotion = dataset[FEATURES_NUM].split("'");
				emotion = predictEmotion[1];
				addInstance(trainingDataSet, features, emotion);
			}
	...
	private void addInstance(Instances trainingDataSet, double[] features,
			String emotion) {
		Instance instance = createInstance(trainingDataSet, features, emotion);
	...
	private Instance createInstance(Instances associatedDataSet,
			double[] features, String Emotion) {
